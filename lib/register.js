	...
	if (req.isAuthenticated()) {
		...
		if (stores.get("users").indexes.get("email").has(args.email)) {
			...
			newUser(args).then(arg => {
				...
				notify("email", stores.get("users").toArray([arg.user])[0], config.template.email.verify, (req.headers["x-forwarded-proto"] ? req.headers["x-forwarded-proto"] + ":" : req.parsed.protocol) + "//" + (req.headers["x-forwarded-protocol"] || req.parsed.host));
			}, e => {
				error(req, res, e);
			});
		}
