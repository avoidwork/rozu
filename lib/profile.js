...
function profile (req, res) {
	...
	if (method === "DELETE") {
		...
		next = () => {
			...
			} else if (method === "PUT" && (typeof data.firstname !== "string" || typeof data.lastname !== "string" || typeof data.email !== "string" || !regex.firstname.test(data.firstname) || !regex.lastname.test(data.lastname) || !regex.email.test(data.email))) {
				res.error(400, config.error.invalid_arguments);
			} else {
				// @todo make it more robust for a patch op, validate the data
