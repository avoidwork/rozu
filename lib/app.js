...
var ROOT_ROUTES = [],
    ...
    app = void 0,
    mta = void 0,
    clientPublish = void 0,
    clientSubscribe = void 0;
...
function receive(req, res) {
	var isCoap = arguments.length <= 2 || arguments[2] === undefined ? false : arguments[2];
	var payload = arguments[3];

	var data = !isCoap ? clone(req.body) : payload,
	...
	if (!isCoap) {
		if (!token || !webhook || config.validate && webhook[1].host.indexOf(req.parsed.hostname) === -1) {
			res.error(401);
		} else if (data === undefined || !regex.payload.test(typeof data === "undefined" ? "undefined" : _typeof(data))) {
			res.error(400);
		} else {
